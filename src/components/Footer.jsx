export default function Footer({ siteStyle, theme, onNavigate }) {
  ...
  return (
    <footer className={`py-10 text-xs transition-colors duration-300 border-t ${
      isMinimal 
        ? 'bg-white border-stone-200 text-stone-600 font-sans' 
        : 'bg-premium-charcoal border-stone-800 text-stone-400 font-serif'
    }`}>
      <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
        <button 
          onClick={() => handleNavClick('home')}
          className={`flex items-center space-x-2 text-xl tracking-widest font-bold bg-transparent border-transparent cursor-pointer ${
            isMinimal ? 'font-sans text-stone-900' : 'font-serif text-white'
          }`}
        >
          <span>MASTALEX</span>
          <span className={`w-1.5 h-1.5 rounded-full ${isMinimal ? 'bg-black' : 'bg-[#c5a880]'}`}></span>
        </button>
